...
import numpy as np
import math
from rotationPreprocess import rotationPreprocess

def buildRotations(n, rand_or_identity,num_rots=None, marker=""):
    print("num_rots: %d" %num_rots)
    num_rots = num_rots or (n-1)
    n_prime = int(n*(n-1)//2*num_rots/(n-1))
    outputs = []

    with tf.variable_scope("Build_Rotations"):

        (indices, values_idxs) = rotationPreprocess(n, num_rots)
        if rand_or_identity:
            print("Initialization: Random")
            thetas = tf.get_variable(initializer=tf.random_uniform([n_prime, 1], 0, 2*math.pi),
                    name="Thetas_RandInit"+marker, dtype=tf.float32)
        else:
            print("Initialization: Identity")
            thetas = tf.get_variable(initializer=tf.zeros([n_prime, 1]),
                    name="Thetas_OnesInit"+marker, dtype=tf.float32)
        cos = tf.cos(thetas)
        sin = tf.sin(thetas)
        nsin = tf.neg(sin)
